...
int initTab(int * tab, int size) {
	...
	if ((tab == NULL) || (size < 0)) {
		...
		for (int i = 0; i < size; i++) {
			tab[i] = 0;  // Le tableau est complété avec uniquement des 0
		}
		return size;
	}
}
...
int* ajoutElementDansTableau(int* tab, int* size, int* nbElts, int element) {
	if (tab == NULL || size == NULL || nbElts == NULL || *size < 0 || *nbElts < 0) return NULL; //Valeurs entrées non valides alors on return NULL
